...
from models.ResUNet import BasicBlock
'''
    UNet 3+
'''
class ResUNet_3Plus(nn.Module):

    def __init__(self, in_channels=3, n_classes=1,feature_scale=4, is_deconv=True, is_batchnorm=True, block = BasicBlock, num_block = [3, 4, 6, 3]):
        super(ResUNet_3Plus, self).__init__()
        ...
        ## -------------Encoder--------------
        self.conv1 = self._make_layer(block, filters[0], num_block[0], 1)

        self.conv2 = self._make_layer(block, filters[1], num_block[0], 2)

        self.conv3 = self._make_layer(block, filters[2], num_block[0], 2)

        self.conv4 = self._make_layer(block, filters[3], num_block[0], 2)

        self.center = self._make_layer(block, filters[4], num_block[0], 2)
    ...
    def forward(self, inputs):
        ...
        hd5 = self.center(h4)  # h5->20*20*1024
